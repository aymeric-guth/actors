...
def defer(callback: Callable, timeout: float = 1.0, logger=None) -> None:
...
class ActorSystem(BaseActor, metaclass=SingletonMeta):
    ...
    def __init__(self) -> None:
        super().__init__(
            pid=ActorSystem.__pid,
            name=self.__class__.__name__,
            parent=ActorSystem.__pid,
        )
    ...
    def _create_actor(
        self, cls: type, *, pid: int, parent: int, name: str = "", **kwargs
    ) -> int:
        ...
        match msg:
            ...
            case Message(sig=Sig.SIGINT):
                ...
                if child is not None and child.pid:
                    self._send(
                        sender=child, receiver=self.pid, msg=Message(sig=Sig.EXIT)
                    )
                ...
                self.logger.error(
                    f"Trying to regenerate Actor(pid={pid}, cls={cls}, parent={parent}, name={name}, kwargs={kwargs})"
                )
            ...
            case Message(sig=Sig.EXIT):
                ...
                if actor is not None:
                    self._send(
                        self,
                        actor.parent,
                        Message(sig=Sig.CHILD_DEINIT, args=actor.pid),
                    )
                    self._registry.unregister(actor.pid)
                    if actor.child:
                ...
                match msg:
                    case MsgCtx(
                        ...
                        self._send(
                            sender=self,
                            receiver=actor.pid,
                            msg=Message(sig=Sig.DISPATCH_ERROR),
                        )
                    case MsgCtx(
                        original_sender=sender,
                        original_recipient=recipient,
                        message=args,
                    ) if isinstance(recipient, str) or isinstance(recipient, type):
                        # self.logger.error(f'{msg=} {actor=}, {recipient=}, {args=}')
                        defer(
                            lambda: self._send(
                                sender=actor, receiver=recipient, msg=args
                            ),
...
def send(to: Union[int, str, type], what: Any) -> None:
    ...
    # ActorNotFound
    return ActorSystem()._send(sender=caller, receiver=to, msg=what)
...
def create(cls: type, *, name: str = "", **kwargs) -> int:
    return ActorSystem()._create_actor(
        cls=cls,
